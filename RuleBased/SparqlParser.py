...
from RuleBased.Triple import Candidate
from RuleBased.Params import ht_conn, numbers_to_display_of_cands
...
class SparqlParser:
    ...
    def execute_var1BGP(self, r_rules_dict, graph):
        for var in self.var1BGP:
            for BGP in self.var1BGP[var]:
                ...
                if BGP[0].startswith("?"):
                    h_idx_list = []
                    t_idx_list = [graph.get_e_idx_by_e_name(BGP[2])]
                    idx_of_var = 0
                else:
                    h_idx_list = [graph.get_e_idx_by_e_name(BGP[0])]
                    t_idx_list = []
                    idx_of_var = 1

                rule_path_list = [[graph.r_id_by_name(BGP[1])]]
                rule_path_list.extend([rule_obj.get_r_id_rule() for rule_obj in r_rules_dict[BGP[1]]])

                rule_path_list = rule_path_list[:100]
    ...
    def execute_var2BGP(self, r_rules_dict, graph):
        ...
        while not BGP_queue.empty():
            ...
            for BGP in self.var2BGP[token]:
                ...
                rule_path_list = [[graph.r_id_by_name(r_name)]]
                # rule_path_list.extend([rule_obj.r_path for rule_obj in r_rules_dict[graph.r_id_by_name(r_name)]])
                rule_path_list.extend([rule_obj.get_r_id_rule() for rule_obj in r_rules_dict[r_name]])

                rule_path_list = rule_path_list[:100]

                print("Rule length 10")
                ...
                if len(h_idx_list) == 0 or len(t_idx_list) == 0:
                    ...
                    for rule in rule_path_list:
                        print("{}".format("->".join(list(map(str,rule)))))
    ...
    def gen_pra_conf_and_rule_path(self, r_rules_dict, rule_model_dict, graph):
        ...
        for cnt, cand in enumerate(self.searched_res):
            ...
            for one_bgp in self.sparql_BGP:
                h_name = one_bgp[0]
                r_idx = graph.r_id_by_name(one_bgp[1])
                t_name = one_bgp[2]
                ...
                if graph.has_fact(h_idx, r_idx, t_idx):
                    ...
                    continue
                t_r_name = graph.get_r_name_by_r_idx(r_idx)
                rule_list = r_rules_dict[t_r_name]
                features, res_path = graph.get_passed_e_r_path(h_idx, t_idx, rule_list)
                pra_conf = rule_model_dict[t_r_name].get_output_prob([features])
    ...
    def sort_cand_obj_list(self, tmp_criterion):
        if tmp_criterion == 'pra':
            print("Sort candidate list by pra score.")
            self.cand_obj_list = sorted(self.cand_obj_list, key=lambda cand: cand.cand_pra_score, reverse=True)
        else:
            print("Sort candidate list by embedding score.")
            self.cand_obj_list = sorted(self.cand_obj_list, key=lambda cand: cand.cand_embedding_score, reverse=True)

    # def get_embedding_conf_and_rule_path(self, r_rules_dict, graph):
    #     e_vec_list, r_vec_list = graph.get_er_embedding()
    #     total_num = len(self.searched_res)
    #     time_start = time.time()
    #     for cnt, cand in enumerate(self.searched_res):
    #         cand_obj = Candidate(cand)
    ...
    #             h_idx = graph.get_e_idx_by_e_name(h_name)
    #             t_idx = graph.get_e_idx_by_e_name(t_name)
    #             h_vec = e_vec_list[h_idx]
    #             t_vec = e_vec_list[t_idx]
    #             r_vec = r_vec_list[r_idx]
    #
    #             if graph.has_fact(h_idx, r_idx, t_idx):
    #                 cand_obj.add_path_idx_for_bgp([[-1, h_idx, r_idx, t_idx, -1]], one_bgp_str)
    ...
    #                     cand_obj.add_pra_conf_for_bgp(1, one_bgp_str)
    #                 else:
    #                     cand_obj.embedding_conf[one_bgp_str] = 1
    #                 continue
    #
    #             cand_obj.get_embedding_conf_for_bgp(h_vec, r_vec, t_vec, one_bgp_str)
    #
    #             rule_list = r_rules_dict[r_idx]
    #             features, res_path = graph.get_passed_e_r_path(h_idx, t_idx, rule_list)
    #
    #             cand_obj.add_path_idx_for_bgp(res_path, one_bgp_str)
    #         cand_obj.cal_can_embedding_score()
    #         print("Candidate: {}/{}.\n".format(cnt + 1, total_num))
    #         print("{}".format(cand_obj.display_rule_path(graph, "embedding")))
    #         self.cand_obj_list.append(cand_obj)
    #
    #         time_end = time.time()
    #         ellapsed = time_end - time_start
    ...
    # def gen_pra_embedding_conf_and_rule_path(self, r_rules_dict, rule_model_dict, graph):
    #     e_vec_list, r_vec_list = graph.get_er_embedding()
    #     total_num = len(self.searched_res)
    #     time_start = time.time()
    #     for cnt, cand in enumerate(self.searched_res):
    ...
    #         cand_obj.cal_can_embedding_score()
    #         print("Candidate: {}/{}.\n".format(cnt + 1, total_num))
    #         print("{}".format(cand_obj.display_rule_path(graph, "pra_embedding")))
    #         self.cand_obj_list.append(cand_obj)
    #
    #         time_end = time.time()
    #         ellapsed = time_end - time_start
    ...
    def display_cands(self, graph):
        self.logger.info("Start printing candidates.")
        res_all = ""
        res_topk = ""

        for idx, cand in enumerate(self.cand_obj_list):
            res_str = "Candidates: {}/{}. conf:{}\n".format(idx + 1, numbers_to_display_of_cands, cand.get_conf("pra"))
            res_str += cand.display_var2entity(self.var_list, graph) + "\n"
            res_str += cand.display_rule_path(graph) + "\n\n"
            if idx < numbers_to_display_of_cands:
                res_topk += res_str
            res_all += res_str

        with open(graph.qe_res_all, "w", encoding="UTF-8") as f:
            f.write(res_all)

        with open(graph.qe_res_topk, "w", encoding="UTF-8") as f:
            f.write(res_topk)
    ...
    -----------
    graph: Graph object
    The graph that stores OneCons_eval.
    '''
